...
export default function OrderDetail({ gps = { status: 'idle', accuracy: null, error: '' } }) {
  ...
  const canPickup = deliveryStatus === 'aceptado' || deliveryStatus === 'recogido' || deliveryStatus === 'asignado externo' || deliveryStatus === 'entregado al operador externo';
    ...
    <div className="page-content detail-page">
      ...
      <div className="detail-grid">
        <div className="detail-stack">
          <section className="panel"><div className="panel-title"><h2>Cliente y destino</h2></div><dl className="data-list"><div><dt>Nombre</dt><dd>{order.customerName}</dd></div><div><dt>Teléfono</dt><dd>{order.customerPhone || '—'}</dd></div><div><dt>Dirección</dt><dd>{order.address}</dd></div><div><dt>Barrio</dt><dd>{order.barrio || '—'}</dd></div>{order.apartment && <div><dt>Apartamento</dt><dd>{order.apartment}</dd></div>}{order.tower && <div><dt>Torre</dt><dd>{order.tower}</dd></div>}{order.floor && <div><dt>Piso</dt><dd>{order.floor}</dd></div>}<div><dt>Referencia</dt><dd>{order.reference || 'Sin referencia'}</dd></div><div><dt>Ubicación</dt><dd>{order.destinationLatitude != null ? 'Confirmada por el cliente' : 'Basada en la dirección escrita'}</dd></div><div><dt>Observaciones</dt><dd>{order.notes || 'Sin observaciones'}</dd></div></dl><div className="quick-actions">{order.phoneLink && <a className="quick-button" href={order.phoneLink}><Phone /> Llamar</a>}{order.whatsappLink && <a className="quick-button" href={order.whatsappLink} target="_blank" rel="noreferrer"><MessageCircle /> WhatsApp</a>}<button type="button" className="quick-button primary" onClick={() => setNavModalOpen(true)}><Navigation /> Navegar</button></div></section>
          <section className="panel"><div className="panel-title"><h2>Productos</h2><span>{(order.items || []).length} referencias</span></div><div className="product-list">{(order.items || []).map((item, index) => <div className="product-row" key={`${item.id || index}-${index}`}><span className="qty">{item.quantity || 1}×</span><div><b>{item.title || item.name}</b>{item.notes && <small>{item.notes}</small>}</div><strong>{money(Number(item.price || 0) * Number(item.quantity || 1))}</strong></div>)}</div>{order.notes && <div className="order-note"><b>Observación general</b><p>{order.notes}</p></div>}</section>
        </div>
