...
import { speakNotification } from '@distrito/shared-ui';

export default function Home() {
  const { profile, settings } = useContext(AuthContext);
  ...
  const [committed, setCommitted] = useState(() => Number(profile?.committed_orders || 0));
  const knownAvailableIds = useRef(new Set());
  const hasLoadedAvailable = useRef(false);
  const hasCapacity = committed < capacity;
  ...
  const load = useCallback(async (silent = false) => {
    ...
    try {
      ...
      setCommitted(Number(availableData.committed ?? profile?.committed_orders ?? currentData.orders?.length ?? 0));
  ...
  }, [profile?.committed_orders]);
  useEffect(() => { load(); }, [load]);
  useEffect(() => {
    const refresh = (event) => { if (event.detail?.event !== 'delivery_location') load(true); };
  ...
  const accept = async (id) => {
    ...
    try {
      ...
      speakNotification('order_accepted', settings || {});
  ...
  return (
    <div className="page-content">
      <section className="page-heading">
        <div><span className="eyebrow"><Radio size={14} /> Sincronizado en vivo</span><h1>{settings?.delivery_heading || 'Pedidos disponibles'}</h1><p>{settings?.delivery_subtitle || `Hola, ${profile?.name || profile?.username}. Puedes llevar ${capacity} pedido${capacity === 1 ? '' : 's'} simultáneamente.`}</p></div>
        <button className="button button-ghost" onClick={() => load()}><RefreshCw size={18} /> Actualizar</button>
      </section>
      {error && <div className="alert alert-error">{error}</div>}
      {!profile?.shift_active && <div className="alert alert-info">Inicia tu turno para recibir, aceptar y transportar pedidos. El sistema verificará GPS, dispositivo y presencia.</div>}
      ...
      {!hasCapacity && <div className="alert alert-info">Tienes {committed} de {capacity} cupos comprometidos, incluyendo reservas realizadas desde el panel administrativo.</div>}
      <section>
